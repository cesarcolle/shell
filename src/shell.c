  ...
  if ((tok = strtok (tok, " ")) != NULL)
      ...
      if (strcmp (tok, "cd") == 0)
	{
	  ...
	  if ((tok = strtok (tok, " ")) != NULL)
	    {
	      reset_path (buffer);
	      // si le chemin donné n'est pas un répertoire
	      if (!isRep (tok))
		{
		  ERROR (0);
		  exit (1);
		}
		//else update of the path
	      int i = 0;
	      while (i < strlen (tok))
		{
...
/**
* Create a only command managed by execv later.
* Combinate the path and the command given by the user in one pointer.
*
*
*/
void
  ...
  while (((c = getchar ()) != EOF) && i < MAX)
    {
      if (c == '\n' && strlen (command) > 1)
	{
	printf("commande noté : %s, chemin : %s \n", command, path);
	  if (!manage_cd (command, path))
	    {
	      int pid = fork ();
	      if (pid == 0)
		{
		  char **tab = str_split (command, ' ', 0);
		  if (!*path){*path = '.';}
		  new_command (tab, path);
		  char *cmd = find_path (tab[0]);
		  tab[0] = strdup (cmd);
		  strcpy (tab[0], cmd);
		  printf("j'execute : %s, tab : %s\n", cmd, *tab);
		  execv (cmd, tab);
		}
	      else
		{
